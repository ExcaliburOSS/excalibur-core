...
When unconfigured, a curated provider chooser (arrow-key **+ type-ahead filter**, grouped Recommended / Subscription or API / API / Local): Kimi K2 (Moonshot, recommended), MiniMax, GLM (Zhipu/Z.ai), Anthropic, OpenAI, Google Gemini, DeepSeek, OpenRouter, Groq, xAI, Cerebras, Together, Fireworks, plus free local **Ollama** (highlighted when detected on PATH/:11434) and a keyless **self-hosted** endpoint. Env-key detection: if a provider's key is already exported, offer it in one keystroke (or float detected providers to the top). **You paste the API key (masked)**; it's saved to the global secrets store `~/.config/excalibur/secrets.env` (`0600`), loaded on launch — `providers.yaml` records only the env var NAME (`apiKeyEnv`), never the value. One pasted key auto-configures a curated good + fast pair. The first-run shell onboarding is ZERO-FRICTION (no "set up now?" consent, no "configure later" — Excalibur is useless without a model; Ctrl-C exits), and it runs a live connection test at the end; `excalibur init`/`models setup` keep a "Configure later" option. Real providers execute live (M2+). `excalibur models setup` reruns this flow; commands needing a model later show the guided chooser (§7), never a low-level error.
