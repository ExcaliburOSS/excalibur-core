...
- **AO5-4..6** remaining: a thin AUTHOR-defined dep-graph surface (the last CC-parity bit — declare named agent steps + deps → compiles to a manifest); dynamic re-planning/self-healing; verification gates as DAG EDGES; ≤1 capped recursion (depth 2, branch-namespace blocker).
