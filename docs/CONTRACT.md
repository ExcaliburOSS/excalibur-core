# Excalibur Core — Engineering Build Contract

> **Status note (1.0.0).** This is the original engineering build contract that
> drove Excalibur Core through milestones **M1–M3, which are now shipped**. The
> **pinned public APIs in §4 remain authoritative** — they are the contract the
> codebase still honors. The milestone-gated _behavioral_ notes below ("M1 uses
> the mock provider", "real providers arrive in M2/M3", "commands are simulated",
> "activates in M3", …) describe the original build sequence, **not the product
> today**: real model providers, the real native agent loop (which reads and
> writes the working tree under the Permission Engine), real command execution,
...
## 1. Scope of M1 (historical — superseded by M2/M3)

OSS-0 → OSS-3 plus a fully working **mock loop**: `init`, `workflows list|explain`, `ask/explain/review`, `patch` lifecycle, `run` with local artifacts/events, `daily`/`weekly-plan` reports, the local **Discovery** flow (`excalibur discovery`, see `docs/spec/discovery-core.md`), the **hybrid extension architecture** (EXT-0→EXT-4 + EXT-7 full; EXT-5 local-compiled-entrypoint only; EXT-8 manifest validation only — see `docs/spec/extensions-spec.md`), and the **out-of-the-box onboarding** (ONB-0→ONB-9 — see `docs/spec/onboarding-core.md`, which OVERRIDES earlier init pins: minimal init by default, `--team`/`--full` modes, `standard-safe` preset, command→workflow mapping, task-intent classifier, per-command local artifacts).

> **What M1 deliberately deferred (and M2/M3 since delivered).** M1 shipped with
> **no real model calls** (only `MockProvider`), **no real agent file
> modification**, and **no real command execution inside runs** (commands carried
> `simulated: true` payloads). All three are now live: **M2** added the real model
> gateway (`anthropic`, `openai-compatible`, `ollama`) and **M3** added the real
> native agent loop, real command/test execution, the Docker sandbox, swarm,
> LSP, and MCP — each gated by the Permission Engine + inline approvals, not by a
> mock. The behavioral pins in §4.3/§4.4/§4.6 marked "M1" record that original
> scaffold; the API shapes they pin are unchanged.
...
- `config.ts`: `excaliburConfigSchema` / `ExcaliburConfig` modeling OSS spec §10 + §14 + work-items §6 + onboarding §2, all sections optional:
  `version?: number`,
  `project?: { name?: string; packageManager?: string; languages?: string[]; frameworks?: string[]; commands?: { test?: string; lint?: string; typecheck?: string; build?: string } }`,
  `commands?: { test?: string; lint?: string; typecheck?: string; build?: string }` (top-level, canonical — `project.commands` is normalized into it),
  `safety?: { preset?: string }` (default `standard-safe`),
...
- `DEFAULT_WORKFLOWS: ReadonlyArray<{ id: string; yaml: string; definition: WorkflowDefinition }>` — the **14** workflows: `ask-repo, review-only, assist, propose-patch, fast-fix, standard-feature, structured-feature, safe-refactor, pr-review, security-review, migration, explore-alternatives, human-gated, discovery`. The three YAML examples in oss-spec §9 are verbatim-normative, as is `discovery.yaml` in `docs/spec/discovery-core.md` §5; `ask-repo` per `onboarding-core.md` §6; author the others in the same style (review-only/assist/propose-patch follow the Enterprise spec §8 examples; `human-gated` = plan → human_approval(required) → agent_work → command_group → human_approval(required) → pull_request).
- `DEFAULT_METHODOLOGIES: ReadonlyArray<{ id: string; yaml: string; definition: Methodology }>` — the **14** methodologies: the 12 of oss-spec §7 (ids in the table) plus `discovery` (verbatim-normative YAML in `docs/spec/discovery-core.md` §4) plus `agentic-agile-light` (`onboarding-core.md` §6); `spec-driven` YAML in oss-spec §8 is verbatim-normative.
...
- `createProvider(name, cfg): ModelProviderAdapter` — `mock` → MockProvider; `anthropic`/`openai-compatible`/`ollama`/`vllm` → the real streaming adapters delivered in M2. _(M1 pin, since superseded: any non-mock type threw `ProviderError` `provider_not_implemented` "real providers arrive in OSS-4 (M2)".)_
...
- `class NativeAgentAdapter implements AgentAdapter` — the real model→tool loop (M3): drives the configured provider through `NATIVE_TOOLS`, **reads and writes the working tree** (path-confined to the workdir, every op gated by the `PermissionEngine` + inline approvals), runs real commands/tests, and emits the live event stream (tool_call → file_read → model_call → file_write → command_started/completed → test_result → patch_generated when role is implementer); the real diff travels in the `patch_generated` payload as `{ diff: string, filesAffected: string[] }`. _(M1 pin, since superseded: this adapter used the MockProvider to produce a **scripted** stream with `{ simulated: true }` commands and **never touched the filesystem**.)_
- `class CustomCommandAdapter implements AgentAdapter` — config shape per oss-spec §15; `detect()` checks the binary exists on PATH; bridges external coding-agent CLIs into the run loop (M3). _(M1 pin, since superseded: `run()` yielded a single `error` event explaining it activated in M3.)_
...
- Init (REVISED by onboarding spec): `generateInitPlan(analysis: RepoAnalysis, opts: { mode: 'minimal' | 'team' | 'full' }): InitPlan` where `InitPlan { files: Array<{ relPath: string; content: string; exists: boolean }>; summaryLines: string[] }`. Minimal (default): ONLY `config.yaml` (onboarding-core.md §2 shape, detected values only — never invent commands or reference non-existent instruction paths) + `instructions/general.md` + `extensions.yaml`; `models/providers.yaml` only when provider setup completed. Team: + instructions/{architecture,testing,security}.md + policies/{standard-safe,sensitive-paths}.yaml + models/{providers,routing}.yaml. Full: exports all built-in catalogs (14 workflows, 14 methodologies, question-packs, prompts, artifacts, policies, reports, roles, command-mappings). `applyInitPlan(repoRoot, plan, opts: { overwrite: boolean }): { written: string[]; skipped: string[] }` — update mode when `.excalibur/` exists (show what would change, confirm before applying).
- Onboarding helpers: `classifyTaskIntent(task: string, analysis: RepoAnalysis, config: ExcaliburConfig): { taskType: 'bugfix'|'feature'|'refactor'|'migration'|'security'|'docs'|'ambiguous'|'alternatives'; sensitive: boolean; sensitiveAreas: string[]; recommendDiscoveryFirst: boolean; recommendedWorkflow: string; recommendedAutonomy: AutonomyLevel; reason: string }` implementing the heuristics of onboarding-core.md §6; `COMMAND_DEFAULTS` const (the command→entity/level/workflow table); `SAFETY_PRESETS` const with `standard-safe` (full permission set §5) used to derive PermissionEngine config.
- Local stores (ONB-8): alongside RunManager, `PatchStore` and `InteractionStore` with `create/get/list` writing `.excalibur/patches/<patch_YYYYMMDD_HHMMSS>/` and `.excalibur/interactions/<int_YYYYMMDD_HHMMSS>/` (files per onboarding-core.md §7, incl. `effective-instructions.md` and `metadata.json` with command/workflow/level/model/sources/warnings/cost/timestamps).
...
Pins: `BUILT_IN_EXTENSIONS: ReadonlyArray<BuiltInExtensionPack>` where `BuiltInExtensionPack = { manifest: ExtensionManifest; contributions: Contribution[] }`. Packs: `core-methodologies` (14 methodologies), `core-workflows` (14 workflows), `discovery-pack` (question packs from DISCOVERY_QUESTION_PACKS + discovery roles as role_definitions + artifact templates refined-ticket/mvp-scope/readiness-assessment + prompt discovery-synthesis), `core-prompts` (pr-summary, code-review), `core-policies` (the `standard-safe` preset per onboarding-core.md §5 — blocked paths include `**/*.p12`, `**/*.pfx`, `.git/**`), `core-reports` (daily-summary, weekly-plan report templates), `core-command-mappings` (work-item-commands mapping mirroring the parser table). All content sourced from `@excalibur/workflow-schema` constants where they exist (single source of truth — packs wrap, never duplicate). A test validates every pack against `extensionManifestSchema` and every contribution against its declarative schema.
...
| Command | Behavior (M1) |
|---|---|
| `init [--team\|--full]` | analyzeRepository → optional one-question provider setup (onboarding-core.md §4; skippable, mock is M1 default) → generateInitPlan(mode) → grouped detection + instruction report → confirm (auto-yes with `--yes`) → applyInitPlan (update mode when `.excalibur/` exists; `--force` overwrites) → the §12 confidence-building final output |
| `models setup` | interactive provider chooser (openai-compatible/anthropic/openrouter/ollama-with-detection/mock/later); env var NAMES only, never key values; writes `.excalibur/models/providers.yaml`; honest M2 note for real providers |
...
| `models list` | providers from `.excalibur/models/providers.yaml` else defaults; real providers (`anthropic`/`openai-compatible`/`ollama`) are live since M2 |
| `ask/explain/review` | builds prompt (selection/file/`--diff` via getLocalDiff) + effective instructions → MockProvider → prints markdown answer AND writes an InteractionStore artifact; maps per COMMAND_DEFAULTS (ask→ask-repo L1, review→review-only L0) |
| `patch "<task>"` | PatchStore artifact set under `.excalibur/patches/<patch-id>/` (input.md, effective-instructions.md, diff.patch, summary.md, metadata.json); prints files affected + summary + diff, then `Apply patch? [y/N]` |
| `run "<task>"` (intent) | classifyTaskIntent decides fast-fix vs standard-feature (or recommends Discovery first for ambiguous, careful for sensitive); shows `Using: <workflow> / Autonomy / Safety: standard-safe` + plan preview + `[Enter] continue [m] change mode [c] cancel`; every run prints the active safety preset |
...
| `pr-create` | real: opens a pull request via the GitHub CLI (`gh pr create`), body from the run's `pr-summary.md`/`summary.md`, flags `--base/--draft/--web/-y` + `[runId]`; requires `gh` on PATH (`pr.ghRequired` otherwise) |
| `cmux` | honest stub: explains which milestone activates it (OSS-10) |
