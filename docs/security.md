...
- Every mutating tool (`write_file`, `run_command`, `apply_patch`) passes the **Permission Engine**: blocked paths (`.env`, secret files, `.git/…`) are hard-denied, mutating tools default to _ask_, and commands outside the allowlist need confirmation.
