...
function engineeringGuidance(role: AgentRole): string[] {
  ...
  return [
    ...
    'To show, open, run, serve or preview a WEB app locally — whether you just built it OR',
    'the user simply asks to SEE it ("show me the web", "enséñame la web", "run the site",',
    '"open it") — ALWAYS call the `preview` tool. It starts the project on localhost and',
    'returns the URL; give that URL to the user. NEVER start a server yourself with',
    'run_command (e.g. `node server.js &`, `npm start`, `python -m http.server`): a',
    'run_command server is reaped when the command settles, so it never stays up — `preview`',
    'is the only correct way, and it keeps the server running for the whole session. Never',
    'tell the user to start a server themselves.',
