...
describe('renderTurnReceipt (light)', () => {
  it('renders an action receipt: headline, metrics line, file list and an apply hint', () => {
    const out = render({
      ...
      metrics: { files: 2, insertions: 40, deletions: 3, inputTokens: 9800, outputTokens: 2600, costCents: 4 },
    ...
    expect(out).toContain('/changes · /replay');
  });
  ...
  it('omits the zero side of a per-file diffstat (added file shows only +N)', () => {
    const out = render({
      ...
      checks: [],
      metrics: { files: 1, insertions: 28, deletions: 0, inputTokens: 100, outputTokens: 50, costCents: 0 },
      nextHint: null,
    } as TurnSummary);
  ...
  it('an answer receipt is just the narrative + a footer (no metrics block)', () => {
    const out = render({
      ...
      changedFiles: [],
      checks: [],
      metrics: { files: 0, insertions: 0, deletions: 0, inputTokens: 1200, outputTokens: 320, costCents: 0 },
      nextHint: null,
    } as TurnSummary);
    expect(out).toContain('It uses selectWorkflow to score intent.');
    expect(out).not.toContain('files');
    expect(out).not.toContain('/changes · /replay');
  ...
  it('a failed receipt leads with the failing check and a fix hint', () => {
    const out = render({
      ...
      changedFiles: [{ path: 'src/net/retry.ts', status: 'modified', insertions: 18, deletions: 6 }],
      checks: [{ label: 'npm test', ok: false, detail: 'exit 1' }],
      metrics: { files: 1, insertions: 18, deletions: 6, inputTokens: 7200, outputTokens: 1900, costCents: 3 },
